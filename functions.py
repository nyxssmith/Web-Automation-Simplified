...
class actions:
    ...
    def get_xpath_as_string_for_body_element(self, element_string, driver):
        ...
        regex_safe_element_string = element_string.replace("(", "\(").replace(")", "\)").replace(".","\.").replace("+","\+").replace(">","\>").replace("<","\<").replace("-","\-")
        ...
        for line in split:
            if line != '':
                ...
                for result in regex_result_open:
                    if result != 'input' and result != 'br' and result != 'hr' and (not ((">" or "<") in result) and "--" not in result):
